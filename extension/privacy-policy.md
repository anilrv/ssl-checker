...
- **Domain registries (RDAP/WHOIS)** — our backend sends the site's registrable domain name directly to whichever registry operates its top-level domain (e.g. Verisign for .com, DNS Belgium for .be) via the public RDAP or WHOIS protocol, to look up public domain registration data (registrar, registration/expiry dates, DNS provider, registered owner organization). This uses ICANN/IANA's own public domain-lookup infrastructure rather than a single third-party API; which registry receives the query depends on the site's domain, the same way the CA revocation query below depends on who issued its certificate.
